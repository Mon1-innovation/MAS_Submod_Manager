...
screen navigation():
    ...
    vbox:
        ...
        textbutton _("Submod管理") action Function(Jump_start)
        textbutton _("设置") action ShowMenu("preferences")
        textbutton _("退出") action Quit()
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    style_prefix "radio"
                    label _("MAS安装目录")
                    if persistent.mas_basedir == None or persistent.mas_basedir == "":
                        text ("未设置")
                    else:
                        text ("当前目录：[persistent.mas_basedir]")

                    textbutton _("更改目录") action Jump("projects_directory_preference")
                vbox:
                    style_prefix "radio"
                    label _("Submod安装")
                    textbutton _("通过压缩包安装Submod") action Call("choose_submod_directory", test=1)
                    textbutton _("通过文件夹安装Submod") action Call("choose_submod_directory", test=2)
...
screen confirm_2(message, ok_action):
